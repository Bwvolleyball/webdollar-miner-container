...
TAG=$(curl -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/webdollar/node-webdollar/releases/latest | jq -r '.tag_name' )-pos100

echo "$DOCKER_TOKEN" | docker login --username bwvolleyball --password-stdin

# shellcheck disable=SC2086
docker build -t bwvolleyball/webdollar-miner-container:latest-pos100 -t bwvolleyball/webdollar-miner-container:$TAG --build-arg WEBDOLLAR=$WEBDOLLAR_SHA --build-arg ARGON=$ARGON_SHA .

docker push bwvolleyball/webdollar-miner-container:latest-pos100
# shellcheck disable=SC2086
